...
function fitFuelUi(){
  ...
  var lab=$("mpgLabel"), box=$("phevBox");
  if(k==="electric"){
    lab.textContent="mi / kWh";
    if(!$("mpgOverride").value||$("mpgOverride").value==="38")$("mpgOverride").value=car&&car.range&&car.kwh?(car.range/car.kwh).toFixed(1):"3.5";
    box.className="hide";
  }else if(k==="phev"){
    lab.textContent="Petrol MPG (after battery)";
    box.className="";
    if(car&&car.range&&!$("evRange").value)$("evRange").value=car.range;
  }else{
    lab.textContent=k==="diesel"?"Diesel MPG":"Petrol MPG";
    box.className="hide";
  }
}
function setCar(c){car=c;if(c&&c.mpg&&$("mpgOverride"))$("mpgOverride").value=c.mpg;fitFuelUi();if(lastTrip)paintResult(lastTrip.title,lastTrip.oneMiles||lastTrip.miles,lastTrip.oneSecs||lastTrip.secs,lastTrip.roads);paintFavs();paintPrices()}
...
function paintPrices(){
  var k=car&&car.kind?car.kind:"diesel";
  var bits=["Diesel "+PRICE.diesel+"p","Petrol "+PRICE.petrol+"p"];
  if(k==="electric"||k==="phev") bits.push("Electric "+PRICE.electric+"p/kWh");
  $("priceLine").textContent=bits.join(" \u00b7 ")+" \u00b7 "+PRICE.date+" UK average";
}
...
if($("evRange"))$("evRange").addEventListener("input",refreshOpts);
