...
function num(v,fb){var n=parseFloat(v);return isFinite(n)?n:(fb||0)}
...
function plateBox(p){p=String(p||"").toUpperCase().replace(/\s+/g," ").trim();return "<div class='platebox"+(p?"":" empty")+"'><span class='gb'>UK</span><span class='reg'>"+(p||"YOUR REG")+"</span></div>"}
function shot(c){return c.photo?("<img src='"+c.photo+"' alt=''/>"):"<div class='addshot'>No photo</div>"}
var STATS=[
  {k:"s060",label:"0–60",unit:"s",low:1,tip:"Lower wins"},
  {k:"bhp",label:"BHP",unit:"",low:0,tip:"Higher wins"},
  {k:"nm",label:"Torque",unit:"Nm",low:0,tip:"Higher wins"},
  {k:"mph",label:"Top speed",unit:"mph",low:0,tip:"Higher wins"},
  {k:"mpg",label:"UK MPG",unit:"",low:0,tip:"Higher wins"},
  {k:"boot",label:"Boot",unit:"L",low:0,tip:"Higher wins"},
  {k:"tank",label:"Range-ish",unit:"L tank",low:0,tip:"Bigger tank helps"}
];
function val(c,k){
  if(k==="range"){var t=num(c.tank),m=num(c.mpg);return t&&m?(t/4.546)*m:0}
  return num(c[k]);
}
function flavour(name){
  var n=String(name||"").toLowerCase();
  if(/volvo/.test(n)) return "Volvo invented the three-point belt. Safety is doing a lap of honour.";
  if(/gr yaris|yaris gr/.test(n)) return "Toyota built a rally car and pretended it was a supermarket runabout.";
  if(/yaris|aygo/.test(n)) return "Toyota reliability. The car will outlive the argument.";
  if(/bmw/.test(n)) return "BMW. The indicator is a myth, the car is not.";
  if(/audi/.test(n)) return "Audi. Quietly judging everyone in the outside lane.";
  if(/mercedes|merc /.test(n)) return "Mercedes. Arrives late and still looks in charge.";
  if(/skoda|škoda/.test(n)) return "Skoda. The punchline that started winning.";
  if(/vauxhall|corsa|astra/.test(n)) return "Vauxhall. Britain’s default setting.";
  if(/ford|fiesta|focus/.test(n)) return "Ford. Has seen things. Will see more.";
  if(/vw|volkswagen|golf/.test(n)) return "VW. Sensible until somebody ordered the GTI.";
  if(/honda|civic|integra/.test(n)) return "Honda. The engine wants a track. The owner wants Tesco.";
  if(/nissan|skyline|gtr|qashqai/.test(n)) return "Nissan. Either a family taxi or a rumour with wheels.";
  if(/land rover|range rover|defender/.test(n)) return "Land Rover. Built for a farm. Lives on a school run.";
  if(/mini/.test(n)) return "Mini. Small car, large personality, tighter parking.";
  if(/tesla/.test(n)) return "Tesla. Silent until the software update.";
  if(/toyota/.test(n)) return "Toyota. Will still be starting in 2041.";
  return "A car with opinions. Some of them even mechanical.";
}
var TWISTS=[
  ...
  {t:"Motorway roadworks. 50 for nine miles.",k:"mpg",low:0,w:"turns it into a saving.",l:"turns it into a mood."},
  {t:"The challenge is a wet grass field after a festival.",k:"nm",low:0,w:"walks out like it paid for a ticket to leave.",l:"is accepting help from a man in a hi-vis."},
...
function scoreKey(c){return kn(c)}
var st={n:5,i:0,a:0,b:0,log:[],house:null};
function cards(){return {a:me(),b:st.house||them()}}
function compare(stat,a,b){
  var va=val(a,stat.k), vb=val(b,stat.k);
  var aw, bw;
  if(stat.low){aw=va>0&&(vb<=0||va<vb);bw=vb>0&&(va<=0||vb<va)}
  else {aw=va>vb;bw=vb>va}
  var draw=!aw&&!bw;
  return {va:va,vb:vb,aw:aw,bw:bw,draw:draw};
}
function roast(winCar,loseCar,lineW,lineL){
  return "<b>"+esc(kn(winCar))+" "+esc(lineW)+"</b><p>"+esc(kn(loseCar))+" "+esc(lineL)+"</p><p class='flv'>"+esc(flavour(kn(winCar)))+"</p>";
}
function tcard(c,mine){
  var rows=STATS.map(function(s){
    var v=c[s.k]; if(s.k==="tank") v=c.tank;
...
function board(){
  return "<div class='scoreline'><span>"+esc(kn(cards().a))+" <b>"+st.a+"</b></span><i>Rd "+Math.min(st.i+1,st.n)+" / "+st.n+"</i><span><b>"+st.b+"</b> "+esc(kn(cards().b))+"</span></div>";
}
function renderIdle(msg){
  var box=el("arena"); if(!box) return;
  var c=cards();
  box.innerHTML=board()+"<div class='duel'>"+tcard(c.a,true)+"<div class='vs'>VS</div>"+tcard(c.b,false)+"</div>"+
    (msg?("<div class='roast'>"+msg+"</div>"):"<p class='copy'>Tap a stat on your card to call it, or hit Twist for a pub-car-park scenario.</p>");
}
function finish(){
  var c=cards();
  var winner=st.a>st.b?c.a:st.b>st.a?c.b:null;
  var title=winner?kn(winner)+" takes the table": "Honours even. Nobody walks home proud.";
  var trip=st.log.length?("<ol class='trip'>"+st.log.map(function(x){return "<li>"+esc(x)+"</li>"}).join("")+"</ol>"):"";
  el("arena").innerHTML=board()+"<div class='roast winend'><h3>"+esc(title)+"</h3><p>"+st.a+" — "+st.b+"</p>"+trip+
    (winner?("<p class='flv'>"+esc(flavour(kn(winner)))+"</p>"):"")+"</div>";
}
function playStat(key,twist){
  if(st.i>=st.n){finish();return}
  var stat=STATS.filter(function(s){return s.k===key})[0]||STATS[1];
  var c=cards();
  var r=compare(stat,c.a,c.b);
  var line=twist||(stat.label+" called");
  var html;
  if(r.draw){
    ...
    st.a+=3; st.b+=1;
    html="<div class='hint'>Round "+(st.i+1)+" · "+esc(stat.label)+" "+r.va+" beats "+r.vb+"</div><h3>"+esc(line)+"</h3>"+roast(c.a,c.b,twist&&twist.w?twist.w:"takes it clean.",twist&&twist.l?twist.l:"can sit this one out.");
    st.log.push(line+" — "+kn(c.a));
  } else {
    st.b+=3; st.a+=1;
    html="<div class='hint'>Round "+(st.i+1)+" · "+esc(stat.label)+" "+r.vb+" beats "+r.va+"</div><h3>"+esc(line)+"</h3>"+roast(c.b,c.a,twist&&twist.w?twist.w:"takes it clean.",twist&&twist.l?twist.l:"can sit this one out.");
    st.log.push(line+" — "+kn(c.b));
  }
  st.i++;
  renderIdle("<div class='win'>"+html+"</div>");
  if(st.i>=st.n) setTimeout(finish,900);
}
function twist(){
  var job=TWISTS[Math.floor(Math.random()*TWISTS.length)];
  var stat=STATS.filter(function(s){return s.k===job.k})[0]||STATS[0];
  stat={k:job.k,label:stat.label,unit:stat.unit,low:job.low};
  playStat(job.k,{t:job.t,w:job.w,l:job.l,label:job.t});
  var box=el("arena"); if(!box) return;
}
function reset(){
...
function bind(){
  ...
  if(el("btnDeal")) el("btnDeal").addEventListener("click",function(e){e.stopImmediatePropagation(); if(st.i>=st.n) reset(); else twist();});
  if(el("btnRecap")) el("btnRecap").addEventListener("click",function(e){e.stopImmediatePropagation(); finish();});
  if(el("btnGame")) el("btnGame").addEventListener("click",function(){setTimeout(reset,30)});
